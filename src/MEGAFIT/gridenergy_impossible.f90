...
subroutine anharmonic_free_energy(p,fct,fcf,qp,dr,temperature,free_energy_thirdorder,free_energy_fourthorder,mw,mem,verbosity)
    !> crystal structure
    ...
    third: block
        ...
        complex(r8), dimension(:,:), allocatable :: buf_ev1,buf_ev2,buf_ev3,buf_ev4
        complex(r8), dimension(:), allocatable :: evp1,evp2,evp3,ptf
        complex(r8) :: c0
        real(r8), dimension(3) :: qv1,qv2,qv3
        real(r8) :: psisq,sigma,om1,om2,om3,n1,n2,n3,f1,f2,prefactor
        ...
        do q2=1,qp%n_full_point
            ...
            do b2=1,dr%n_mode
                ...
                !call zgeru(dr%n_mode, dr%n_mode, (1.0_r8,0.0_r8), nuvec2(:,b2,q2), 1, nuvec1(:,b1,q1), 1, evp1, dr%n_mode)
                do b3=1,dr%n_mode
                    evp2=0.0_r8
                    call zgeru(dr%n_mode, dr%n_mode*dr%n_mode, (1.0_r8,0.0_r8), buf_ev3(:,b3), 1, evp1, 1, evp2, dr%n_mode)
                    !call zgeru(dr%n_mode, dr%n_mode*dr%n_mode, (1.0_r8,0.0_r8), nuvec2(:,b3,q3), 1, evp1, 1, evp2, dr%n_mode)
                    evp2=conjg(evp2)
        ...
        call mem%deallocate(buf_ev4,persistent=.false.,scalable=.false.,file=__FILE__,line=__LINE__)
...
!     type(lo_phonon_dispersions), intent(in) :: dr
!     !> temperature
!     real(flyt) :: temperature
!     !> free energies
!     real(flyt), intent(out) :: free_energy_thirdorder,free_energy_fourthorder
!     !> mpi helper
!     type(lo_mpi_helper) :: mw
!
!     real(flyt), dimension(:,:), allocatable :: en3,en4
!     real(flyt) :: omthres,sigma
!     integer :: index_gamma
!
!     init: block
!         integer :: i
...
!         if ( mw%talk ) write(*,*) 'REVISIT SMEARING',__FILE__
!         sigma=lo_mean(dr%default_smearing)
!         omthres=dr%omega_min*0.5_flyt
!         index_gamma=-1
!         do i=1,qp%n_irr_point !q_irr
...
!     third: block
!         complex(flyt) :: c0
!         real(flyt), dimension(3) :: v0,qv1,qv2,qv3,omega
!         real(flyt) :: psisquare,doublepsi,prefactor
!         real(flyt) :: n1,n2,n3,f0,f1,f2,f3
!         integer, dimension(3) :: gi
!         integer :: q1,q2,q3,b1,b2,b3,ctr
!         ! buffers for speed
!         complex(flyt), dimension(:,:,:,:,:,:), allocatable :: cwrk,cdwrk
!         real(flyt), dimension(dr%n_mode) :: pbuf1,pbuf2,pbuf3,ombuf1,ombuf2,ombuf3
!         real(flyt), parameter :: onethird=1.0_flyt/3.0_flyt
!
...
!
!         ctr=0
! !        prefactor=-(lo_hbar_J**2)/48.0_flyt
! !        prefactor=prefactor/lo_j2ev ! get it to eV
!         prefactor=1.0_flyt/48.0_flyt
!         do q1=1,qp%n_irr_point
!         do q2=1,qp%n_full_point
...
!                     f1=n1*n2+n1+onethird
!                     f1=3*f1*real(1.0_flyt/( omega(1)+omega(2)+omega(3)+lo_imag*sigma ))
!                     f2=2*n1*n3-n1*n2+n3
!                     f2=3*f2*real(1.0_flyt/( omega(1)+omega(2)-omega(3)+lo_imag*sigma ))
!                 else
!                     f1=0.0_flyt
!                     f2=0.0_flyt
!                     psisquare=0.0_flyt
...
!                 !    doublepsi=real(c0)
!                 !    f3=6*(n1*(n2+1)+0.25_flyt)
!                 !    f3=f3*real(1.0_flyt/( omega(3)+lo_imag*sigma ))
!                 !else
!                 !    doublepsi=0.0_flyt
!                 !    f3=0.0_flyt
!                 !endif
...
!         real(flyt) :: psi,prefactor,n1,n2,f1
!         integer :: q1,q2,b1,b3,i,j,l
!         ! buffers for speed
!         complex(flyt), dimension(:,:), allocatable :: egv,flat_cwrk
!         real(flyt), dimension(:,:), allocatable :: flat_m,flat_r2,flat_r3,flat_r4
!         real(flyt), dimension(dr%n_mode) :: dumom1,dumom3,dump1,dump3
!         integer(i8) :: ctrlong
...
!         egv=0.0_flyt
!         ! the unit
!         !prefactor=(lo_hbar_J**2)/32.0_flyt
!         !prefactor=prefactor/lo_j2ev
!         prefactor=1.0_flyt/32.0_flyt
!
!         ! start integrating
!         ctrlong=0
!         do q1=1,qp%n_irr_point
...
!             do b1=1,dr%n_mode
!             ctrlong=ctrlong+1; if ( mod(ctrlong,int(mw%n,flyt)) .ne. mw%r ) cycle
!             do b3=1,dr%n_mode
...
!         en3=en3/real(qp%n_full_point,flyt)/real(fc%na,flyt)
!         en4=en4/real(qp%n_full_point,flyt)/real(fc%na,flyt)
...
!         real(flyt), dimension(3,nquartet), intent(in) :: flv2,flv3,flv4
!         real(flyt), dimension(81,nquartet), intent(in) :: fm
!         complex(flyt), dimension(81,na*na*na*na), intent(inout) :: fcw
!         integer, dimension(4,nquartet), intent(in) :: fai
!         !> four frequencies
!         real(flyt), dimension(4), intent(in) :: omega
!         !> four eigenvectors
!         complex(flyt), dimension(na*3,4), intent(in) :: egv
!         !> three q-points
!         real(flyt), dimension(3), intent(in) :: q2,q3,q4
!         !> the matrix element
!         complex(flyt), intent(out) :: psi
!
...
!         real(flyt), dimension(3), intent(in) :: omega
!         !> the eigenvectors
!         complex(flyt), dimension(fc%na*3), intent(in) :: egv1,egv2,egv3
!         !> the two q-vectors that matter
!         real(flyt), dimension(3), intent(in) :: q2,q3
!         !> work array
!         complex(flyt), dimension(3,3,3,fc%na,fc%na,fc%na), intent(inout) :: egvprod
!         ! Matrix element
!         complex(flyt), intent(out) :: psi
!
!         complex(flyt) :: expiqr,c0
!         real(flyt), dimension(3) :: rv2,rv3
!         real(flyt) :: iqr,omegaprod
...
! !        complex(flyt), dimension(fc%na*3), intent(in) :: egv1,egv2,egv3
! !        !> the two q-vectors that matter
! !        real(flyt), dimension(3), intent(in) :: q1,q2
! !        !> work arrays
! !        complex(flyt), dimension(3,3,3,fc%na,fc%na,fc%na), intent(inout) :: egvprod1,egvprod2
! !        !> double psi
! !        complex(flyt), intent(out) :: doublepsi
! !        !
! !        integer :: atom1,atom2,atom3,i,j,k,k1,k2,k3,t
! !        complex(flyt) :: expiqr,c0,c1
! !        real(flyt), dimension(3) :: rv2,rv3
! !        real(flyt), parameter :: enhet=2.367755374064161E51_flyt
! !        real(flyt) :: iqr,omegaprod1,omegaprod2
! !
