...
subroutine parse(opts)
    ...
    call cli%add(switch='--output_format', switch_ab='-of', &
                 help='Selects output format. 1 is VASP, 2 is Abinit, 3 is LAMMPS, 4 is FHI-Aims, 5 is Siesta, 6 is QE. Default 1.', &
                 required=.false., act='store', def='1', choices='1,2,3,4,5,6', error=lo_status)
    if (lo_status .ne. 0) stop
